...
# 예외처리
# 무장이나 표현식이 문법적으로 올바르다 할지라도, 실행하려고 하면 에러를 일으킬 수 있다.
...
# try 문 안에서 error 가 발생할 경우 pass를 사용하여 오류를 그냥 지나가도록 작성할 수 있다.
...
class MyError(Exception):  #Exception을 상속 받았다. 
     pass

def say_nick(nick):
     if nick =='돼지':
          raise MyError()   #오류를 던지는 것
